...
int XMLNode::SaveFileData(char *pBuffer, int nBufLen)
{
	if (m_pNodelist.size() > 0)
	{
		sprintf_s(pBuffer, nBufLen, "<%s>\n", m_acTag);

		int ii = 0;
		list<XMLNode*>::iterator cur;

		for (cur = m_pNodelist.begin(); ii < m_pNodelist.size(); ii++, cur++)
		{
			if ((*cur) != NULL)
			{
				(*cur)->SaveFileData(&pBuffer[strlen(pBuffer)], nBufLen - strlen(pBuffer));
			}
		}

		sprintf_s(&pBuffer[strlen(pBuffer)], nBufLen - strlen(pBuffer), "</%s>\n", m_acTag);
	}
	else
	{
		sprintf_s(&pBuffer[strlen(pBuffer)], nBufLen - strlen(pBuffer), "<%s>%s</%s>\n", m_acTag, m_acValue, m_acTag);
	}

	return strlen(pBuffer);
}

int XMLNode::WriteFiledDataToFile(char *pFileName, int nLen, char *pRecordFile, int nRecordLen)
{
	int nRet = 0;
	FILE *pFile = NULL;
	char acBuffer[1024*2] = { 0 };

	SaveFileData(acBuffer, sizeof(acBuffer));

	fopen_s(&pFile, pFileName, "wb");

	if (pFile != NULL)
	{
		fwrite(acBuffer, sizeof(char), strlen(acBuffer), pFile);
...
int XMLNode::GetValue(int *pValue, char *pTag, int nTagLen)
{
	...
	for (ii, cur = m_pNodelist.begin(); ii < m_pNodelist.size(); ii++, cur++)
	{
		if ((*cur)->IsSameTag(pTag, nTagLen))
		{
			...
			{
				if (memcmp(acValue, "True", strlen("True")) == 0)
				{
					*pValue = 1;
				}
				else
